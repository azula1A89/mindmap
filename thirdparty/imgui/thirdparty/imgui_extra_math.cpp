...
//------------------------------------------------------------------------------
# ifndef __IMGUI_EXTRA_MATH__
# define __IMGUI_EXTRA_MATH__
...
{
    ImVec2 O = start + 0.5f * ssize;
    float ex = end.x;// -_offset.x;
    float ey = end.y;// -_offset.y;
    float tx = ex - O.x;
    float ty = ey - O.y;
    float pi = 3.1415926f;
    float C = atanf(ssize.y / ssize.x);
    float D = pi - C;
    float A = pi + C;
    float B = 2 * pi - C;
    float T = pi + atan2f(-ty, -tx);

    ImVec2 point;
    if (T > C && T < D)
    {
        point = { 0.5f * ssize.y * tx / ty, 0.5f * ssize.y };
    }
    else if (T > D && T < A)
    {
        point = { -0.5f * ssize.x, 0.5f * ssize.x * ty / fabs(tx) };
    }
    else if (T > A && T < B)
    {
        point = { 0.5f * ssize.y * tx / fabs(ty), -0.5f * ssize.y };
    }
    else
    {
        point = { 0.5f * ssize.x, 0.5f * ssize.x * ty / tx };
    }
    out = point + O;
}
void calculate_line_vertex(ImVec2 start, ImVec2 ssize, ImVec2 end, ImVec2 esize, ImVec2& sout, ImVec2& eout)
{
    calculate_rectangle_contact_point(start, ssize, end + 0.5f * esize, sout);
    calculate_rectangle_contact_point(end, esize, start + 0.5f * ssize, eout);
}

void create_triangle_vertex(ImVec2 from, ImVec2 p3, ImVec2& p1, ImVec2& p2, float angle, float arrow_sz)
{
    const float pi = 3.1415926535f;
    angle = pi * (180 - ImClamp(angle, 1.0f, 89.0f)) / 180;
    ImVec2 base,midline,origin;
    midline = ImNormalized(p3 - from);
    origin = p3 - midline * arrow_sz;
    base = p3 - origin;
    p1 = ImRotate(base, ImCos(angle), ImSin(angle)) + origin;
    p2 = ImRotate(base, ImCos(-angle), ImSin(-angle)) + origin;
}
...
std::pair<ImVec2, float> get_point_on_polyline(float pos, std::vector<ImVec2> &polyline)
{
    ...
    std::map<float, std::pair< float, std::pair<ImVec2, ImVec2>>> segments;
    std::vector<float> segment_lengths;
    for (size_t i = 1; i < n; i++)
    {
        float segment_length = ImLength( polyline[i-1] - polyline[i]);
        segment_lengths.push_back(segment_length);
        total_length += segment_length;
    }
    float accumulate_length = 0;
    for (size_t i = 1; i < n; i++)
    {
        accumulate_length += segment_lengths[i-1];
        segments[accumulate_length/total_length] = {segment_lengths[i-1]/total_length, {polyline[i-1], polyline[i]}};
    }
    auto search = std::find_if(segments.begin(), segments.end(), [pos](auto v){ return (pos <=v.first); });
    return std::make_pair(ImLerp(search->second.second.second, search->second.second.first, (search->first - pos)/(search->second.first)), total_length);
}
//------------------------------------------------------------------------------
 ...
 ImVec2 ImRect_ClosestPoint(const ImRect& rect, const ImVec2& p, bool snap_to_edge, float radius)
{
 ...
 ImVec2 ImRect_ClosestPoint(const ImRect& rect, const ImRect& other)
{
    ImVec2 result;
